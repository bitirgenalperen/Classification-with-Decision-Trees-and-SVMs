...
def MetricsStatDisplay(predict_labels, actual_labels, phase):
    truePositive = np.sum(np.logical_and((predict_labels == actual_labels) , (actual_labels == 1)))
    trueNegative = np.sum(np.logical_and((predict_labels == actual_labels) , (actual_labels == 0)))
    falsePositive = np.sum(np.logical_and((predict_labels == 1) , (actual_labels == 0)))
    falseNegative = np.sum(np.logical_and((predict_labels == 0) , (actual_labels == 1)))
    
    accuracy = (trueNegative + truePositive)/len(actual_labels)
    ...
    print("-------------------------------------------------------")
    print("-------------------------------------------------------")
    print("**Printing the confusion matrix and metrics for " + phase + "**")
    print("******************** Actual Label *********************")
    print("                            +                           -")
    print("Predicted     +             "+str(truePositive)+"                       "+str(falsePositive))
    print("Label         -             "+str(falseNegative)+"                           "+str(trueNegative))
    print("*******************************************************")
    print("-------------------------------------------------------")
    print("-------------------------------------------------------")
    print("*******************************************************")
